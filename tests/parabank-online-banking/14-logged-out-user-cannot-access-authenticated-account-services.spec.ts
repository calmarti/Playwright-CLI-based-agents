...
test.describe('ParaBank Online Banking', () => {
  test('Logged out user cannot access authenticated account services', async ({ page }, testInfo) => {
    ...
    const accountNumber = (await page.locator('#accountTable a').filter({ hasText: /^\d+$/ }).first().textContent())?.trim() ?? '';
    ...
    for (const path of ['overview.htm', 'transfer.htm', 'billpay.htm']) {
      await page.goto(path);
      await expect(page.getByRole('heading', { name: 'Customer Login' })).toBeVisible();
      await expect(page.getByRole('heading', { name: 'Account Services' })).not.toBeVisible();
      await expect(page.getByRole('link', { name: accountNumber })).not.toBeVisible();
    }
  });
});
