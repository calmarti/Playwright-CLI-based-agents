...
test.describe('ParaBank Online Banking', () => {
  test('Bill Pay validates required fields and account confirmation', async ({ page }, testInfo) => {
    ...
    await page.locator('input[name="verifyAccount"]').fill('9998');
    await page.locator('input[name="amount"]').fill('10.00');
    await page.getByRole('button', { name: 'Send Payment' }).click();

    await expect(page.getByText('The account numbers do not match.')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Bill Payment Complete' })).not.toBeVisible();
